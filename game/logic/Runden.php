...
set_error_handler(function($severity, $message, $file, $line) use (&$result) {
    $result["error"] = "PHP Error: $message in $file on line $line";
    echo json_encode($result);
    exit;
});
#
set_exception_handler(function($e) use (&$result) {
    $result["error"] = "PHP Exception: " . $e->getMessage();
    echo json_encode($result);
    exit;
});
#
